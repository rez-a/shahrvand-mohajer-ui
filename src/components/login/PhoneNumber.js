...
import detectedLanguageKeyboardUser from 'helper/convertNumbers2English';
...
const PhoneNumber = ({
  ...
  const handleSendVerifyCode = async () => {
    ...
    if (phoneNumberRegex.test(convertNumbers2English(phoneNumber))) {
      setLoading(true);
      try {
  ...
  const handleKeyDown = (e) => {
    e.key === 'Enter' && handleSendVerifyCode();
  };
