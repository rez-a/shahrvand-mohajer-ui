      ...
      >
        <div className="px-4 flex items-center">
          <span
            className={`bg-rose-500 mr-auto text-xs text-white p-2 rounded-xl rounded-br-none font-bold ${
              DiscountPrice > 0 ? 'visible' : 'invisible'
            }`}
          >
            {DiscountPrice}%
          </span>
        ...
        <div class="px-6 pt-4 pb-2">
          <Link
            ...
            class="inline-block bg-gray-200 rounded-full px-3 py-1 text-xs text-gray-700 opacity-75 hover:opacity-100 transition-all duration-200 cursor-pointer hover:underline"
          >
            #{SideGroupName}
          </Link>
        </div>
        <div className="text-sm px-6 font-bold">
          ...
          {DiscountPrice > 0 && (
            <span className="mr-4 text-rose-500 whitespace-nowrap">
              <span>
                {SellPrice - (SellPrice * DiscountPrice) / 100}
              </span>
              تومان
            </span>
          )}
